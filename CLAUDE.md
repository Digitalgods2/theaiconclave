...
# Run the full test suite (431 tests)
python -m pytest
...
5. **Terminate** — orchestrator writes a `final_results` row, status becomes `completed` / `failed` / `cannot_resolve`. `_post_finalize_hooks` then runs the non-blocking extras (failure-cause tags, trajectory export, artifact capture). Sandboxes are cleaned up; orphan sandboxes are swept on service startup (`app/main.py` lifespan).
...
- `services/orchestrator.py` — mode flows + persistence helpers (`_record_message`, `_record_run_*`). The longest and most load-bearing file (~1400 lines).
...
- `api/` — FastAPI routers (`tasks`, `agents`, `git`, `uploads`, `settings`, `help`, `health`).
- `dashboard/` — single-page vanilla-JS app served at `/`. `dashboard.js` is ~5000 lines; modularization is a known "Next" item but not yet acted on.
...
### Persistence
SQLite at `<user_data_root>/switchboard.db`. **WAL mode + `busy_timeout=30s` + `with_retry()` on heavy write paths** are deliberate hardening for the worker/retention/API-call concurrency triangle — don't strip them. Schema lives in `app/database.py`; see `docs/DATABASE_SCHEMA.md` for the table reference.
