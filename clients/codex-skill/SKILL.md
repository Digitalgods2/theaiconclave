...
description: Use this skill when the user asks to consult, deliberate with, or get input from other AI agents (Antigravity, Claude), record a decision, fetch a prior decision, continue a deliberation thread, or answer a paused conclave task. Trigger phrases include "ask the conclave", "convene the conclave", "get a second opinion", "what would Claude/Antigravity think", "have Claude and Antigravity debate this", "deliberate on this", "compare your answer with another AI", "I want a third opinion", "record my decision", "/decide", "what was decided", "what did the conclave decide", "/decision", "continue the conclave thread", "/continue this thread", "show the thread", "answer the paused task", "the conclave is waiting on me". The skill submits a task to The AI Conclave Switchboard (a local FastAPI service at 127.0.0.1:8787) which orchestrates multi-AI deliberation across Codex, Antigravity, and Claude Code, then renders the transcript and final answer.
---

# The AI Conclave Switchboard — Codex skill

You are **Codex**, the user's primary CLI agent for this session. The AI Conclave Switchboard is a local service running at `127.0.0.1:8787` that lets you consult Antigravity and Claude Code as peers, deliberate as part of a 3-AI conclave, record the user's decisions, and continue threaded deliberations. Use it when the user wants a second opinion, a multi-AI conclave, or cross-AI critique of an answer.
...
- `antigravity` — Google Antigravity CLI (the supported Gemini successor)
...
|---|---|---|---|
| "ask the conclave", "convene the conclave", "have them deliberate" | `conclave` | all three (codex, antigravity, claude-code) | full mesh |
| "get a second opinion", "what would X say" | `consult` | codex primary, named agent consultant | codex drafts, consultant critiques |
| "have Antigravity and Claude debate this" | `conclave` | antigravity, claude-code | two-AI |
| "let Claude handle this" | `resolve` | claude-code primary | offloads to claude-code |
| "compare with Antigravity" | `conclave` | codex, antigravity | two-AI |

**Default primary in consult mode is `codex`**, not `claude-code` — because you (Codex) are the agent the user is talking to right now; you draft the answer and ask a peer to critique it. Do not silently make Claude or Antigravity the primary unless the user names them.
...
```bash
python -m switchboard_conclave health
```
...
```bash
python -m switchboard_conclave --invoked-by codex run <mode> <agents_csv> "<question>"
```
...
python -m switchboard_conclave --invoked-by codex run conclave codex,antigravity,claude-code "Should I use Postgres or MongoDB for v1?"

# Quick second opinion (codex primary, antigravity consultant)
python -m switchboard_conclave --invoked-by codex run consult codex,antigravity "Review this approach: ..."

# Two-AI debate (antigravity + claude-code)
python -m switchboard_conclave --invoked-by codex run conclave antigravity,claude-code "Debate this:"
```
...
- **If the task is `awaiting_user_input`**, the agents asked a clarifying question. Show the question to the user and ask them to answer it. Then resume with `python -m switchboard_conclave --invoked-by codex answer <task_id> "<their answer>"`.
...
```bash
python -m switchboard_conclave --invoked-by codex decide <task_id|latest> "<decision text>"
```
...
```bash
python -m switchboard_conclave --invoked-by codex decision <task_id|latest>
```
...
```bash
python -m switchboard_conclave --invoked-by codex continue <parent_task_id|latest> "<new question>"
```
...
```bash
python -m switchboard_conclave --invoked-by codex thread <task_id|latest>
```
...
```bash
python -m switchboard_conclave --invoked-by codex answer <task_id|latest> "<the user's answer>"
python -m switchboard_conclave --invoked-by codex wait <task_id>
```

The `answer` command supports `-` as the answer text to read from stdin — useful for piping shell command output as the answer (e.g., `pytest 2>&1 | python -m switchboard_conclave --invoked-by codex answer latest -`).
