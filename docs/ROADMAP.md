...
| Test suite | Covers protocol, modes, threading, retention, attachments, sandbox, sandbox-inline, judge, db concurrency, export tracking, exporter, provenance, doc export, openrouter adapter, settings API, orphan reaper, confidence aggregate |
...
3. **Decision Memory retrieval** — auto-surface relevant past `docs/decisions/` records + `final_results` rows as a "Prior Art" section in new task contexts. Operationalizes the charter's existing decision-record mandate by making the historical record actively observable instead of passively stored. Read-only augmentation; no new permissions. Phase 2.5 of the post-DR plan on `tsk_01KRSW6AS3M66B4RRJE3JFAPRV` — ships after confidence-weighted synthesis (now shipped). (Capability addition; will get a decision record when picked up.)

4. **Trim Tier 2 after export (opt-in)** — the retention policy says Tier 2 is "retain indefinitely until exported." With export tracking now in place, an optional retention amendment could allow Tier 2 trim after an explicit export, freeing the corresponding `final_results` row. (Operability item.)

5. **Modularize dashboard.js** — file is approaching 2000 lines; still maintainable but ripe for splitting into per-view modules without adopting a framework. Defer until it actively bites.

6. **Inbox tagging** — letting the user attach freeform tags to tasks would scale browsing better than filters alone once the inbox has hundreds of tasks.
