...
| Test suite | Covers protocol, modes, threading, retention (incl. Tier 2 trim), attachments, sandbox, sandbox-inline, judge, db concurrency, export tracking, exporter, provenance, doc export, openrouter adapter, settings API, orphan reaper, confidence aggregate, decision memory |
